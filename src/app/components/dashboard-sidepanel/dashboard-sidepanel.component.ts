...
export class DashboardSidepanelComponent implements OnInit, OnDestroy {
  ...
  initializeDashboard() {
    this.dashboardService
      ...
      .getPupupChange()
      .pipe(takeUntil(this.destroy$))
      .subscribe((showPopup: boolean) => {
        this.showAssigningPopup = showPopup;
      });
  }
  assignTeachers() {
    const userDetails = this.commonService.loginedUserInfo;
    const selectedTeachers: string[] = [];// pass selected teachers id
    this.authService.updateUserInfo({ id: userDetails._id, assignedTo: selectedTeachers });
    this.dashboardService.setPopupChangeValue(false);
  }
}
